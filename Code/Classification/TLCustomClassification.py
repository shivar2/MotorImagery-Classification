...
from Code.base import detect_device, load_data_object, \
    cut_compute_windows, split_into_train_valid, plot, get_results
...
def tl_classifier_phase1(train_set_all,
                  save_path,
                  model,
                  double_channel=True,
                  device='cpu'):
...
def run_model(data_load_path, double_channel, model_load_path, params_name, save_path):
    ...
    clf = tl_classifier_phase1(train_set_all,
                        model=model,
                        save_path=save_path,
                        double_channel=double_channel,
                        device=device)
