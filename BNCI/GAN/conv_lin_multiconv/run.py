...
from BNCI.GAN.conv_lin_multiconv.model import Generator, Discriminator
from GAN.util import weight_filler
import torch
...
EEG_data = load_concat_dataset(
            path='../../../data-file/hgd-raw/' + str(subj_ind),
            preload=True,
            target_name=None,
            )
...
for i_block in range(i_block_tmp,n_blocks):
    ...
    train_tmp = discriminator.model.downsample_to_block(Variable(torch.from_numpy(train_set),volatile=True),discriminator.model.cur_block).data.cpu()
    ...
    for i_epoch in range(i_epoch_tmp,block_epochs[i_block]):
        ...
        batches = get_balanced_batches(train_set.shape[0], rng, True, batch_size=n_batch)
        iters = int(len(batches)/n_critic)

        for it in range(iters):
            for i_critic in range(n_critic):
                train_batches = train_tmp[batches[it*n_critic+i_critic]]
                batch_real = Variable(train_batches,requires_grad=True)

                z_vars = rng.normal(0,1,size=(len(batches[it*n_critic+i_critic]),n_z)).astype(np.float32)
                z_vars = Variable(torch.from_numpy(z_vars),volatile=True)
                batch_fake = Variable(generator(z_vars).data,requires_grad=True)

                loss_d = discriminator.train_batch(batch_real,batch_fake)
                assert np.all(np.isfinite(loss_d))
            z_vars = rng.normal(0,1,size=(n_batch,n_z)).astype(np.float32)
            z_vars = Variable(torch.from_numpy(z_vars),requires_grad=True)
        ...
        if i_epoch%100 == 0:
            ...
            z_vars = Variable(torch.from_numpy(z_vars_im),volatile=True)
