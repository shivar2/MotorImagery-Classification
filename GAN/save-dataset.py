...
def load_data(path):
    ...
    for time in start_times:
        ...
        Fz = np.asarray(C4.tolist() + np.asarray([temp[2]]).tolist())
        Pz = np.asarray(C4.tolist() + np.asarray([temp[2]]).tolist())
    data = np.asarray(np.asarray([C3]).tolist() + np.asarray([Cz]).tolist() + np.asarray([C4]).tolist()
                      + np.asarray([Fz]).tolist() + np.asarray([Pz]).tolist())
...
def short_time_fourier_transform(data):
    ...
    frequency, time, spec = signal.stft(x=data, fs=sampling_frequency, nperseg=window_size, nfft=512,
...
if __name__ == "__main__":
    path = "../mne_data/MNE-bnci-data/database/data-sets/001-2014/"
